...
class UniformReliableBroadcastTest : public testing::Test {
protected:
...
TEST_F(UniformReliableBroadcastTest, SmokeTest1) {
  SetupHosts(0);

  SpinLock mutex;
  std::vector<Bytes> delivered(3);
  size_t received = 0;
  auto callback = [&delivered, &mutex, &received](URBIdentifier identifier,
                                                  Bytes message) noexcept {
    std::lock_guard lock(mutex);
    ++received;
    delivered[identifier.peer] = std::move(message);
  };

  UniformReliableBroadcast urb(*runtime_, callback, hosts_);

  runtime_->SetTransport(urb.GetReliableTransportCallback(), hosts_,
                         GetReliableRetryPolicy());
  auto &transport = runtime_->GetTransport();
  transport.Start(runtime_->GetTimerService(), runtime_->GetEventLoop());

  auto futures = urb.Broadcast(Bytes(1, 'a'));
...
TEST_F(UniformReliableBroadcastTest, SmokeTest2) {
  SetupHosts(1);
  ...
  auto futures = urb.Broadcast(Bytes(1, 'b'));
  All(std::move(futures)).Get();
...
TEST_F(UniformReliableBroadcastTest, SmokeTest3) {
  SetupHosts(2);
  ...
  auto futures = urb.Broadcast(Bytes(1, 'c'));
  All(std::move(futures)).Get();
